...
export default function ProductItems({id, data}:ProductItemsPropsType){

    const [isFavourite, setIsFavourite] = useState<boolean>(false);
    const [isHovered, setIsHovered] = useState<boolean>(false);

    return(
        <div onMouseEnter={()=>setIsHovered(true)} onMouseLeave={()=>setIsHovered(false)}  className="flex flex-col justify-center items-center aspect-[441/603] md:gap-[18.37%] md:pt-[8.62%] md:pb-[10%] hover:border-0 hover:rounded-[10px] hover:bg-[#F7F6F4] hover:shadow-[0_4px_10px_0_rgba(0, 0, 0, 0.1)]">
 
                    <button onClick={()=>{setIsFavourite(!isFavourite)}} className={`${isHovered ? 'opacity-[100%] pointer-events-auto' : 'opacity-0 pointer-event-none'} cursor-pointer bg-[#FFFFFF] md:w-[9.3%] ml-auto md:mr-[6%] flex justify-center items-center aspect-square rounded-full`}>
