...
const ProductCard: React.FC<ProductCardProps> = ({ product, onClick, sortTag }) => {
  ...
  const getTagType = () => {
    if (product.recommended) return "recommended";
    if (product.newArrival) return "new arrival";
    if (product.newCollection) return "new collection";
    return "none";
  };

  const tagType = getTagType();
  const hasValidTagType = tagType !== "none";

  let validItem = true;
  if (sortTag !== "default" && sortTag !== "all") {
    validItem = sortTag.toLowerCase() === tagType;
  }

  // To match ProductItems exactly, we render tags if they match or if we're not sorting.
  // Actually, ProductItems uses 'data.tagType'. Since we determine it here, we will just use it.
  ...
  return (
    ...
    >
      <div className="w-full flex items-center shrink-0">
        <div className={`hidden md:flex md:min-w-[25.85%] px-0 md:px-[2%] aspect-69/18 md:aspect-114/30 md:ml-[6.36%] bg-[#431A1A]/25 justify-center items-center rounded-[20px] ${sortTag === "default" && !hasValidTagType ? "md:hidden" : (!hasValidTagType ? "md:hidden" : "")}`}>
          <h3 className="uppercase text-[10px] md:text-[12px]">{tagType}</h3>
        </div>

        <div className={`md:hidden flex justify-center items-center bg-[#431A1A]/25 rounded-[20px] px-[5.5px] py-[3px] ${sortTag == 'default' && !hasValidTagType ? 'hidden' : (!hasValidTagType ? 'hidden' : '') }`}>
          <h3 className="uppercase text-[8px] md:text-[10px]">
            {tagType !== 'none' ? tagType : ''}
          </h3>
        </div>
      ...
      <div className="w-full flex flex-col justify-end items-center gap-[4px] md:gap-[9px] shrink-0 px-2 mt-auto">
        <p className="text-[10px] md:text-[13px] leading-[19.6px] tracking-[0%] text-[#6E6E6E] m-0">{displayColor}</p>
        <h3 className="w-full font-baskerville text-[13px] md:text-[24px] leading-[14px] md:leading-[19.52px] tracking-[0%] text-[#000000] text-center line-clamp-2">{product.productName}</h3>
        {product.diamondType && (
          <p className="font-baskerville text-[10px] md:text-[16px] leading-[20px] tracking-[0%] text-[#000000] m-0">{product.diamondType}</p> 
        )}
      </div>
    </div>
  );
