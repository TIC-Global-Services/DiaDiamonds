...
export default function CollectionItem({name, color, image}:CollectionItemProp):JSX.Element {
  const router = useRouter();
  const [isFavourite, setIsFavourite] = useState<boolean>(false);

  return (
    <div className="w-full md:w-[24%] md:grow shrink-0 cursor-pointer" >
      ...
      <div className="w-full aspect-square bg-[#f9f9f9] pb-[13px] relative rounded-[20px] overflow-hidden">
        ...
        <div className="absolute  top-0 right-0 px-7 py-7 ">
            <button onClick={(e)=> {e.stopPropagation(); setIsFavourite(!isFavourite)}}  className="w-10 h-10 rounded-full flex justify-center items-center bg-[#FFFFFF] ml-auto hover:cursor-pointer  hover:scale-110 active:scale-100 ">
              {
